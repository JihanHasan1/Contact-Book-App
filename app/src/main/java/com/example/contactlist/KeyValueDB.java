...
public class KeyValueDB extends SQLiteOpenHelper {
    ...
    public boolean updateValueByKey(String key, String value) {
        ...
        int nr = 0;
        try{
            nr = db.update(TABLE_KEY_VALUE, cv, KEY + "=?", new String[] { key });
        }catch (Exception e){
            handleError(db, e);
            try {
                nr = db.update(TABLE_KEY_VALUE, cv, KEY + "=?", new String[]{key});
    ...
    public String getValueByKey(String key) {
        ...
        Cursor res;
        try{
            res = db.rawQuery("SELECT * FROM " + TABLE_KEY_VALUE + " WHERE " + KEY + "='" + key + "'", null);
        }catch (Exception e){
            handleError(db, e);
            res = db.rawQuery("SELECT * FROM " + TABLE_KEY_VALUE + " WHERE " + KEY + "='" + key + "'", null);
        }
        if(res.getCount()>0){
            res.moveToNext();
            return res.getString(1); //get values of column-01 which is values in our case
        }
        return null;
    }
    //
